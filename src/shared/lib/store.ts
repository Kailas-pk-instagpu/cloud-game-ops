...
// ===== Wallet store (customers + loyalty + transactions) =====
import {
  CustomerWallet, WalletTransaction, MOCK_CUSTOMER_WALLETS, MOCK_WALLET_TRANSACTIONS,
  tierFromPoints, LOYALTY_TIERS,
} from './mock-data';

interface WalletState {
  wallets: CustomerWallet[];
  transactions: WalletTransaction[];
  getByBranch: (branchId: string) => CustomerWallet[];
  getTransactions: (walletId: string) => WalletTransaction[];
  topUp: (walletId: string, amount: number, note?: string) => void;
  chargeWallet: (walletId: string, amount: number, note?: string) => void;
  addPoints: (walletId: string, points: number, note?: string) => void;
  redeemPoints: (walletId: string, points: number) => { discount: number };
  awardSettlementPoints: (walletId: string, usageCost: number) => void;
}
...
export const useWalletStore = create<WalletState>()(
  persist(
    (set, get) => ({
      ...
      getByBranch: (branchId) => get().wallets.filter(w => w.branchId === branchId),
      getTransactions: (walletId) =>
        get().transactions.filter(t => t.walletId === walletId)
          .sort((a, b) => b.timestamp.localeCompare(a.timestamp)),
      topUp: (walletId, amount, note) => set((s) => ({
        wallets: s.wallets.map(w => w.id === walletId
          ? { ...w, balance: w.balance + amount, lastActivity: new Date().toISOString() }
          : w),
        transactions: [{
          id: `tx-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
          walletId, type: 'topup', amount, note: note || 'Top-up',
          timestamp: new Date().toISOString(),
        }, ...s.transactions],
      })),
      chargeWallet: (walletId, amount, note) => set((s) => ({
        wallets: s.wallets.map(w => w.id === walletId
          ? { ...w, balance: Math.max(0, w.balance - amount), lastActivity: new Date().toISOString() }
          : w),
        transactions: [{
          id: `tx-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
          walletId, type: 'charge', amount: -amount, note,
          timestamp: new Date().toISOString(),
        }, ...s.transactions],
      })),
      addPoints: (walletId, points, note) => set((s) => ({
        wallets: s.wallets.map(w => w.id === walletId ? { ...w, points: w.points + points } : w),
        transactions: [{
          id: `tx-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
          walletId, type: 'bonus', amount: 0, pointsDelta: points,
          note: note || 'Bonus points', timestamp: new Date().toISOString(),
        }, ...s.transactions],
      })),
      redeemPoints: (walletId, points) => {
        const discount = Math.floor(points / 100);
        set((s) => ({
          wallets: s.wallets.map(w => w.id === walletId
            ? { ...w, points: Math.max(0, w.points - points) }
            : w),
          transactions: [{
            id: `tx-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
            walletId, type: 'redeem', amount: discount, pointsDelta: -points,
            note: `Redeemed ${points} pts for RM ${discount}`,
            timestamp: new Date().toISOString(),
          }, ...s.transactions],
        }));
        return { discount };
      },
      awardSettlementPoints: (walletId, usageCost) => {
        const wallet = get().wallets.find(w => w.id === walletId);
        if (!wallet) return;
        const tier = tierFromPoints(wallet.points);
        const points = Math.floor(usageCost * LOYALTY_TIERS[tier].multiplier);
        if (points <= 0) return;
        get().addPoints(walletId, points, `Session reward (${LOYALTY_TIERS[tier].label} ×${LOYALTY_TIERS[tier].multiplier})`);
      },
    }),
    { name: 'gpu-cloud-wallets' },
  ),
);

// ===== Feedback store =====
import { Feedback, MOCK_FEEDBACK } from './mock-data';

interface FeedbackState {
  feedback: Feedback[];
  addFeedback: (f: Omit<Feedback, 'id' | 'timestamp'>) => Feedback;
  getByBranch: (branchId: string) => Feedback[];
...
export const useFeedbackStore = create<FeedbackState>()(
  persist(
    (set, get) => ({
      ...
      getByBranch: (branchId) => get().feedback.filter(f => f.branchId === branchId)
        .sort((a, b) => b.timestamp.localeCompare(a.timestamp)),
      getAverageRating: (branchId) => {
        const list = branchId ? get().feedback.filter(f => f.branchId === branchId) : get().feedback;
        if (!list.length) return 0;
        return Math.round((list.reduce((s, f) => s + f.rating, 0) / list.length) * 10) / 10;
      },
    }),
    { name: 'gpu-cloud-feedback' },
  ),
);
